...
`[TODO]` further EDA backends (KiCad…) on the same service model, bridge session & last-readback reporting.

Implemented today: the page is a two-pane **multi-EDA service registry** mirroring Agents & tools — the left list manages one card per EDA backend service (JLCircuit today; further backends slot in as new entries), the right pane shows the selected service. The JLCircuit card owns the whole bridge service: the **listen address** (loopback-only, persisted into the runtime settings, applied on the next bridge start), a **supervised child process** started and stopped from the page (status chip: starting / running · PID / stopped / failed, with the bridge's own stderr surfaced when it dies), and layered **monitoring**: an automatic TCP probe (every 5 s while the page is open) distinguishes offline / online / not-yet-probed without disturbing the bridge's single WebSocket client, and an explicit **connection test** performs the protocol-level `status` round-trip — reporting protocol version, the capabilities the bridge itself declares (shown only after such a report, never claimed on its behalf), and the registered project IDs, warning when the list is empty because the EDA extension's `hello` is rejected without one. A port that answers while the process was not started by the app is labeled as externally owned.
